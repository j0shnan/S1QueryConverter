#!/usr/bin/env python3
##########################################################################################
## Convert WL queries in a spreadsheet from SentinelOne DeepVis QL 1.0 to 2.0
##########################################################################################
...
##### Provide path to .xlsx; sheet containing the data; file you'd like to be written:
input_file = ''
input_sheet = ''
output_file = ''
...
##### Define logical f() for replacement mapping and build RE pattern && sub from the mapping
...
def escape_special_chars_inside_quotes(text: str) -> str:
    ...
    def nested_func_replacing_specd_chars(m: re.Match) -> str:
        ...
        #  b) escape backslashes, but skip Unicode \uXXXX sequences
        escaped = re.sub(
            r'(?<!\\)\\(?!u[0-9A-Fa-f]{4})',
            r'\\\\',
            escaped
...
def map_fields_in_matches(text: str, mapping: dict[str,str]) -> str:
    if not isinstance(text, str):
        return text
    qry_contains_pattern_matching_regex = re.compile(r'\smatches\s*\"((?:\\.|[^\"\\])*)\"|\"\(\?i\)')
...
def convert_v1_to_v2(input_text):
    if pd.isna(input_text):
        return input_text
    ##### Field Mapping QL v1 to v2 
    s1QLv1_to_s1QLv2_field_mapping = {
        ...
        #Logins
        'LoginAccountDoconvert_v1_to_v2': 'event.login.accountDoconvert_v1_to_v2',
        ...
        'LoginTgtDoconvert_v1_to_v2Name': 'event.login.tgt.doconvert_v1_to_v2Name',
        ...
        'LogoutTgtDoconvert_v1_to_v2Name': 'event.logout.tgt.doconvert_v1_to_v2Name',
    ...
    # return sentinelOne_QL_v2_converted_query
    return final_query


for input_column, output_column in zip(input_columns,output_columns):
    df[output_column] = df[input_column].apply(convert_v1_to_v2)
